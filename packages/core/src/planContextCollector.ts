...
export function buildPlanModeSystemMessage(context: PlanContextBundle): string {
  ...
  parts.push('  "overview": string,                    // REQUIRED - Rich markdown overview (2-4 paragraphs)');
  ...
  parts.push('    "category": string                   // One of: "setup", "core", "testing", "deploy", "refactor", "config"');
  parts.push('  }>,');
  parts.push('  "risks": string[],');
  parts.push('  "architecture_diagram": string | null, // Optional mermaid diagram (flowchart, sequence, etc.)');
  ...
  parts.push('}');
  parts.push('');
  parts.push('OVERVIEW GUIDANCE:');
  parts.push('- Write 2-4 paragraphs of rich markdown explaining the approach');
  parts.push('- Reference specific files and modules from the project context');
  parts.push('- Use **bold** for key terms, `code` for file paths and symbols');
  parts.push('- Use bullet lists for key points');
  parts.push('- Make it read like a technical design brief, NOT a generic description');
  parts.push('');
  parts.push('ARCHITECTURE_DIAGRAM GUIDANCE:');
  parts.push('- Include ONLY when the plan involves multiple components/services interacting');
  parts.push('- Use valid mermaid syntax (flowchart TD, sequenceDiagram, etc.)');
  parts.push('- Keep diagrams concise (max 15 nodes)');
  parts.push('- Set to null for simple plans (single file edits, config changes, etc.)');
  parts.push('- Example: "flowchart TD\\n  A[User Request] --> B[API Gateway]\\n  B --> C[Auth Service]"');
  parts.push('');
  parts.push('STEP CATEGORY VALUES:');
  parts.push('- "setup": Environment, dependencies, configuration');
  parts.push('- "core": Main feature implementation');
  parts.push('- "testing": Tests, validation, verification');
  parts.push('- "deploy": Build, deploy, CI/CD changes');
  parts.push('- "refactor": Code cleanup, restructuring');
  parts.push('- "config": Configuration files, settings');
  parts.push('');
